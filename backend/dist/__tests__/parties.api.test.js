...
describe("Parties API", () => {
    ...
    describe("GET /parties/:partyId", () => {
        it("returns 200 and party when it exists", async () => {
            const create = await (0, supertest_1.default)(app_1.app).post("/parties").send(validPartyBody).expect(201);
            const partyId = create.body.data.id;
            const res = await (0, supertest_1.default)(app_1.app).get(`/parties/${partyId}`).expect(200);
        ...
        it("returns 404 when party does not exist", async () => {
            const res = await (0, supertest_1.default)(app_1.app).get("/parties/nonexistent-id-12345").expect(404);
            expect(res.body.ok).toBe(false);
    ...
    describe("PATCH /parties/:partyId/locations", () => {
        ...
        it("returns 400 when body has neither pickup nor destination", async () => {
            const create = await (0, supertest_1.default)(app_1.app).post("/parties").send(validPartyBody).expect(201);
            const partyId = create.body.data.id;
            await (0, supertest_1.default)(app_1.app).patch(`/parties/${partyId}/locations`).send({}).expect(400);
        });
    });
    describe("POST /parties/:partyId/join", () => {
        it("allows a user to join and returns 201 with ride info", async () => {
            ...
            const after = await (0, supertest_1.default)(app_1.app).get(`/parties/${partyId}`).expect(200);
