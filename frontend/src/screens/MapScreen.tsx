...
export const MapScreen = ({
  ...
  const handlePlaceSelection = async (placeId: string, description: string, isStart: boolean) => {
    if (isStart) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={[styles.header, { zIndex: 10 }]} pointerEvents="auto">
        <View style={styles.headerLeft}>
          <TouchableOpacity
            ...
            onPress={() => { setIsMenuOpen(!isMenuOpen); if(onMenuPress) onMenuPress(); }}
            activeOpacity={0.7}
        ...
        <View style={styles.headerNav}>
          <TouchableOpacity
            ...
            onPress={() => { if(onSettingsPress) onSettingsPress(); }}
            activeOpacity={0.7}
            hitSlop={{ top: 20, bottom: 20, left: 20, right: 20 }}
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => { if(onProfilePress) onProfilePress(); }}
            activeOpacity={0.7}
            hitSlop={{ top: 20, bottom: 20, left: 20, right: 20 }}
          >
            <LinearGradient colors={["#1B5E20", "#2E7D32"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.avatarGradient}>
              <Text style={styles.avatarText}>{userName && userName.length > 0 ? userName[0].toUpperCase() : "?"}</Text>
            </LinearGradient>
      ...
      {/* Search Panel Modal */}
      <Modal visible={showSearchPanel} transparent animationType="slide" onRequestClose={() => setShowSearchPanel(false)}>
        <SafeAreaView style={styles.searchPanelContainer}>
          <ScrollView style={styles.searchPanelContent} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.searchInputContainer}>
              ...
              {activeSearchField === "start" && startSuggestions.length > 0 && (
                <View style={styles.suggestionsContainer}>
                  {loadingSuggestions ? (
                    <ActivityIndicator size="small" color="#155dfc" style={{ padding: 10 }} />
                  ) : (
            ...
            <TouchableOpacity style={styles.searchButton} onPress={() => setShowSearchPanel(false)} disabled={isSearching}>
      ...
      {!showSearchPanel && (startLocation || endLocation) && (
        <View style={styles.locationOverlay}>
          <Text style={styles.locationOverlayText}>
            {startLocation && endLocation ? startLocation + " -> " + endLocation : startLocation ? "From: " + startLocation : "To: " + endLocation}
          </Text>
      ...
      >
        ...
        {startCoords && (
          <Marker coordinate={startCoords} title="Start" description={startLocation} pinColor="#1B5E20" />
        )}

        {/* Marker for end location */}
        {endCoords && (
          <Marker coordinate={endCoords} title="Destination" description={endLocation} pinColor="#f44336" />
        )}
      </MapView>

      {/* Footer with Actions */}
      <View style={styles.footer} pointerEvents="auto">
        <TouchableOpacity style={styles.searchLocationButton} onPress={() => setShowSearchPanel(true)} activeOpacity={0.8}>
          <Text style={styles.searchLocationButtonIcon}>🔍</Text>
          <Text style={styles.searchLocationButtonText}>Search Locations</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.primaryButton} onPress={() => { if(onViewRideGroups) onViewRideGroups(); }} activeOpacity={0.8}>
          <Text style={styles.primaryButtonIcon}>👥</Text>
          <Text style={styles.primaryButtonText}>View Available Ride Groups</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.secondaryButton} onPress={() => { if(onCreateRideGroup) onCreateRideGroup(); }} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 12, backgroundColor: "#ffffff", shadowColor: "#000000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  headerLeft: { flexDirection: "row", alignItems: "center", gap: 12 },
  menuButton: { width: 40, height: 40, justifyContent: "center", alignItems: "center", borderRadius: 10, backgroundColor: "#ffffff" },
  ...
  settingsButton: { width: 40, height: 40, justifyContent: "center", alignItems: "center", borderRadius: 10 },
  settingsIcon: { fontSize: 20 },
  profileButton: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 12, paddingVertical: 8 },
  avatarGradient: { width: 40, height: 40, borderRadius: 20, justifyContent: "center", alignItems: "center" },
  avatarText: { fontSize: 18, fontWeight: "700", color: "#ffffff" },
  userName: { fontSize: 14, color: "#364153", fontWeight: "500", maxWidth: 150 },
  menuOverlay: { flex: 1, backgroundColor: "rgba(0, 0, 0, 0.5)", justifyContent: "flex-start" },
  menuContent: { backgroundColor: "#ffffff", marginTop: 80, marginHorizontal: 16, borderRadius: 12, overflow: "hidden", shadowColor: "#000000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 8, elevation: 5 },
  menuItem: { paddingVertical: 14, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: "#e5e7eb" },
  menuItemText: { fontSize: 14, color: "#364153", fontWeight: "500" },
  mapContainer: { flex: 1, backgroundColor: "#e5e7eb", justifyContent: "center", alignItems: "center" },
  map: { flex: 1, width: "100%" },
  mapPlaceholder: { alignItems: "center", gap: 8 },
  mapPlaceholderText: { fontSize: 32 },
  mapSubtext: { fontSize: 14, color: "#6b7280" },
  footer: { backgroundColor: "#ffffff", paddingHorizontal: 16, paddingVertical: 16, borderTopWidth: 1, borderTopColor: "#0000001a", shadowColor: "#000000", shadowOffset: { width: 0, height: -2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 4, gap: 12 },
  primaryButton: { backgroundColor: "#1B5E20", borderRadius: 10, paddingVertical: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, shadowColor: "#000000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  primaryButtonIcon: { fontSize: 20 },
  primaryButtonText: { color: "#ffffff", fontSize: 16, fontWeight: "600" },
  secondaryButton: { backgroundColor: "#ffffff", borderRadius: 10, borderWidth: 2, borderColor: "#1B5E20", paddingVertical: 16, alignItems: "center", justifyContent: "center" },
  secondaryButtonText: { color: "#1B5E20", fontSize: 16, fontWeight: "600" },
  availabilityText: { fontSize: 16, color: "#4a5565", textAlign: "center", fontWeight: "500" },
  searchLocationButton: { backgroundColor: "#1B5E20", borderRadius: 10, paddingVertical: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 12 },
  searchLocationButtonIcon: { fontSize: 18 },
  searchLocationButtonText: { color: "#ffffff", fontSize: 16, fontWeight: "600" },
  searchPanelContainer: { flex: 1, backgroundColor: "#f9fafb" },
  searchPanelContent: { flex: 1, paddingHorizontal: 16, paddingVertical: 16 },
  searchHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 24 },
  ...
  searchLabel: { fontSize: 14, fontWeight: "600", color: "#364153", marginBottom: 8 },
  searchInput: { backgroundColor: "#ffffff", borderRadius: 10, paddingHorizontal: 16, paddingVertical: 12, fontSize: 14, color: "#1e2939", borderWidth: 1, borderColor: "#e5e7eb" },
  searchButton: { backgroundColor: "#155dfc", borderRadius: 10, paddingVertical: 16, alignItems: "center", justifyContent: "center", marginBottom: 12 },
  searchButtonText: { color: "#ffffff", fontSize: 16, fontWeight: "600" },
  clearButton: { backgroundColor: "#ffffff", borderRadius: 10, borderWidth: 2, borderColor: "#e5e7eb", paddingVertical: 16, alignItems: "center", justifyContent: "center" },
  clearButtonText: { color: "#999", fontSize: 16, fontWeight: "600" },
  locationOverlay: { position: "absolute", top: 16, left: 16, right: 16, backgroundColor: "#ffffff", borderRadius: 12, padding: 12, shadowColor: "#000000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 5, zIndex: 1 },
  locationOverlayText: { fontSize: 14, fontWeight: "600", color: "#1e2939", marginBottom: 12 },
  locationOverlayButtons: { flexDirection: "row", gap: 8 },
  editLocationButton: { flex: 1, backgroundColor: "#155dfc", borderRadius: 8, paddingVertical: 8, alignItems: "center" },
  editLocationButtonText: { color: "#ffffff", fontSize: 12, fontWeight: "600" },
  clearLocationButton: { flex: 1, backgroundColor: "#f0f0f0", borderRadius: 8, paddingVertical: 8, alignItems: "center" },
  clearLocationButtonText: { color: "#999", fontSize: 12, fontWeight: "600" },
  suggestionsContainer: { position: "absolute", top: "100%", left: 0, right: 0, backgroundColor: "#ffffff", borderRadius: 8, marginTop: 4, borderWidth: 1, borderColor: "#e5e7eb", maxHeight: 200, zIndex: 10 },
